...
'game','gang','gape','garb','gash','gasp','gate','gave','gawk','gawp','gaze','gear','gift','gild','gimp','gird','give','glom','glow','glue','glug','glut','gnaw','goad','golf','gone','gong','goof','gore',
...
function findNext() {
	...
	while(!found && start < end) {
		...
		if( (currDist < closestDist) && !prevData.includes(start)) {
			// print('found closer word');
			closestWord = data[start];
			closestDist = currDist;
			closestWordIndex = start;
			if(currDist < 3 && !prevData.includes(start)){ // this word is one letter or two letters apart?
				// prevData.push(word);
	...
	// print("closest word index is "+closestWordIndex);
	word = split(closestWord, '');
